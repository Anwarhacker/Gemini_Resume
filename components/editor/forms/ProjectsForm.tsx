        ...
        <div className="space-y-6 p-6 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800">
            {data.projects.map((proj, idx) => (
                <div key={proj.id} className="p-5 border border-slate-200 rounded-xl bg-slate-50 relative group">
                    ...
                    <h4 className="text-xs font-bold uppercase text-slate-400 mb-3 tracking-wider">Project #{idx + 1}</h4>
                    <div className="grid grid-cols-1 gap-4 mb-4">
                        <input placeholder="Project Title" className="form-input rounded-lg border-slate-300" value={proj.name} onChange={e => change(proj.id, 'name', e.target.value)} />
                        <input placeholder="Tech Stack (e.g. React, Node.js)" className="form-input rounded-lg border-slate-300" value={proj.technologies} onChange={e => change(proj.id, 'technologies', e.target.value)} />
                        <input placeholder="Project Link / GitHub" className="form-input rounded-lg border-slate-300" value={proj.link} onChange={e => change(proj.id, 'link', e.target.value)} />
                    </div>
                    <textarea placeholder="Project Description" className="form-textarea w-full rounded-lg border-slate-300 h-24" value={proj.description} onChange={e => change(proj.id, 'description', e.target.value)} />
                </div>
            ))}
